...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    boolean existsByNameIgnoreCaseAndCategoryCategoryId(String name, Long categoryId);

    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM Product p WHERE LOWER(p.name) = LOWER(:name) AND LOWER(p.category.categoryName) = LOWER(:categoryName)")
    boolean existsByNameAndCategoryNameIgnoreCase(@Param("name") String name, @Param("categoryName") String categoryName);
}
